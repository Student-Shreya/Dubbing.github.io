...
// /**
//  * Sends a video file to the server for multimodal transcription and translation (Dubverse Job).
//  * @param {File} videoFile - The video file object.
//  * @param {string} targetLanguage - The language code to translate into.
//  * @param {string} sourceLanguage - The language code of the spoken audio in the video.
//  * @returns {Promise<object>} Object containing transcribed text, translated subtitles, and job status.
//  */
// export const localizeVideo = async (videoFile, targetLanguage, sourceLanguage) => {
//     // Check if the input is a File object before proceeding
//     if (!(videoFile instanceof File)) {
//         throw new Error("Video Localization requires a valid File object for upload.");
//     }

//     console.log(`Sending video file ${videoFile.name} for localization to ${targetLanguage}...`);
    
//     const formData = new FormData();
//     // 🛑 FIX: Ensure the video file is appended correctly under the 'video' field name
//     formData.append('video', videoFile); 
//     formData.append('targetLanguage', targetLanguage);
//     formData.append('sourceLanguage', sourceLanguage);
...
/**
 * Sends a video URL to the server for dubbing/subtitling.
 * NOTE: Sends JSON, as the server handles the download (yt-dlp).
 * @param {string} videoUrl - The URL of the YouTube video.
 * @param {string} targetLanguage - The language code to translate into.
 * @param {string} sourceLanguage - The language code of the spoken audio in the video.
 * @returns {Promise<object>} Object containing transcribed text and subtitles.
 */
export const localizeVideo = async (videoUrl, targetLanguage, sourceLanguage) => {
    // Check if the input is a string (URL)
    if (typeof videoUrl !== 'string' || !videoUrl.startsWith('http')) {
        throw new Error("Video Localization requires a valid YouTube/HTTP URL.");
    }

    console.log(`Sending video URL ${videoUrl} for localization to ${targetLanguage}...`);
    
    // Server expects JSON for URL submission
    const response = await fetch(`${API_BASE_URL}/localize/video`, { 
        method: 'POST', 
